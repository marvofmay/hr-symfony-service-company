...
final readonly class ImportEmployeesFacade
{
    ...
    public function handle(UploadedFile $file): array
    {
        ...
        try {
            $user = $this->security->getUser()->getEmployee();
            ...
            if (count($errors) > 0) {
                $message = $this->messageService->get('employee.import.error', [], 'employees');
            ...
            try {
                ...
                $this->commandBus->dispatch(new CreateImportCommand(kindEnum: ImportKindEnum::IMPORT_POSITIONS, statusEnum: ImportStatusEnum::PENDING, file: $file, user: $user));
                $handleStamp = $this->queryBus->dispatch(new GetImportByFileQuery($file));
                $import = $handleStamp->last(HandledStamp::class)->getResult();
                $this->commandBus->dispatch(new ImportEmployeesCommand($import->getUUID()->toString()));
            ...
            if (ImportStatusEnum::DONE === $import->getStatus()) {
                $message = $this->messageService->get('employee.import.success', [], 'employees');
            ...
            $message = sprintf('%s %s', $this->messageService->get('employee.import.error', [], 'employees'), $this->messageService->get($error->getMessage()));
