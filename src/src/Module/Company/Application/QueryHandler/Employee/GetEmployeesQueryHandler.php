        ...
        if (!empty($filters)) {
            ...
            if (array_key_exists('deleted', $filters)) {
                switch ($filters['deleted']) {
                    case 0:
                        $queryBuilder = $queryBuilder->andWhere($queryBuilder->expr()->isNull('r.'.Employee::COLUMN_DELETED_AT));
                        break;
                    case 1:
                        $this->entityManager->getFilters()->disable('soft_delete');
                        $queryBuilder = $queryBuilder->andWhere($queryBuilder->expr()->isNotNull('r.'.Employee::COLUMN_DELETED_AT));
                        break;
                }
            } else {
                $queryBuilder = $queryBuilder->andWhere($queryBuilder->expr()->isNull('r.'.Employee::COLUMN_DELETED_AT));
            ...
            if (array_key_exists('phrase', $filters) && !empty($filters['phrase'])) {
                $queryBuilder = $queryBuilder->andWhere(
                    $queryBuilder->expr()->orX(
                        $queryBuilder->expr()->like('LOWER(r. '.Employee::COLUMN_FIRST_NAME.')', ':searchPhrase'),
                        $queryBuilder->expr()->like('LOWER(r.'.Employee::COLUMN_LAST_NAME.')', ':searchPhrase'),
                    )
                )
