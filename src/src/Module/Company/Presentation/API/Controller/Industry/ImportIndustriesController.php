...
final class ImportIndustriesController extends AbstractController
{
    ...
    ): JsonResponse
    {
        ...
        try {
            ...
            if (count($errors) > 0) {
                return new JsonResponse(['message' => $this->translator->trans('industry.import.error', [], 'industries'), 'errors' => UploadFileErrorTransformer::map($errors)], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            ...
            if ($import->getStatus() === ImportStatusEnum::DONE) {
                return new JsonResponse(['message' => $this->translator->trans('industry.import.success', [], 'industries'), 'errors' => [],], Response::HTTP_CREATED);
            } else {
                $importLogs = $askImportLogsAction->ask($import);
                $errors = ImportLogErrorTransformer::map($importLogs);

                return new JsonResponse(['message' => $this->translator->trans('industry.import.error', [], 'industries'), 'errors' => $errors,], Response::HTTP_UNPROCESSABLE_ENTITY);
            }
