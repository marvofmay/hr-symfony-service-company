...
use App\Module\Company\Presentation\API\Action\ContractType\AskContractTypeAction;
...
class GetContractTypeController extends AbstractController
{
    ...
    public function get(string $uuid, AskContractTypeAction $askContractTypeAction): JsonResponse
    {
        try {
            ...
            return new JsonResponse(['data' => $askContractTypeAction->ask($uuid)], Response::HTTP_OK);
        } catch (\Exception $error) {
            $message = sprintf('%s. %s', $this->messageService->get('contractType.view.error', [], 'contract_types'), $error->getMessage());
