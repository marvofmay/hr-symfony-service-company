...
class ImportEmployeesController extends AbstractController
{
    ...
    public function import(
        ...
        try {
            ...
            $createFileAction->execute($fileName, $uploadFilePath, $security->getUser()->getEmployee());

            //ToDo:: start - move code to file "getFileUUIDAction"
            $handleStamp = $this->queryBus->dispatch(new GetFileByNamePathAndKindQuery($fileName, $uploadFilePath, FileKindEnum::IMPORT_XLSX));
            $fileUUID = $handleStamp->last(HandledStamp::class)->getResult();
            //ToDo:: stop

            //ToDo:: make file "createImport" - save record in "import" table

            //ToDo:: make file "getImportUUIDAction" like "getFileUUIDAction"

            //ToDO:: pass uuid to ImportDTO($importUUID) instead ImportDTO($uploadFilePath, $fileName)

            $importEmployeesAction->execute(new ImportDTO($uploadFilePath, $fileName));
